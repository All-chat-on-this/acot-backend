...
public interface UserConfigService extends IService<UserConfigDO> {
     ...
     * @param secretKey Secret key for the API key
     * @return Test response with success/failure and any results
     */
    ConfigTestVO testConfig(UserConfigDO config, Long userId, String secretKey);
    ...
    /**
     * Set the availability and update the last used time of a configuration
     *
     * @param configId  The configuration ID to update
     * @param available The availability to set
     */
    void setAvailableAndUpdateLastUsedTime(Long configId, boolean available);
} 